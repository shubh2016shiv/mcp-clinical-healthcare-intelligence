...
async def test_analytics_tools():
    ...
    print("-" * 80)
    try:
        request = ConditionAnalysisRequest(group_by="condition", limit=5)
        ...
        if result.groups:
            for group in result.groups[:3]:
                print(
                    f"  - {group.condition_name}: {group.condition_count} cases ({group.patient_count} patients)"
                )
    except Exception as e:
        print(f"✗ Error: {e}")
...
async def test_medication_tools():
    ...
    print("-" * 80)
    try:
        request = MedicationHistoryRequest(include_drug_details=True, limit=5)
...
async def test_drug_tools():
    ...
    print("-" * 80)
    try:
        request = DrugClassAnalysisRequest(group_by="therapeutic_class", limit=5)
