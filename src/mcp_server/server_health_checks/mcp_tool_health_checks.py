"""Tool-specific server_health_checks checks for MCP server.
...
class ToolIntegrityChecker:
    """Specialized checker for tool-related server_health_checks."""
            ...
            for tool_name, tool_class in self.tools.items():
                try:
                    ...
                    if tool_name == "PatientTools":
                        ...
                        timeline_spec = importlib.util.find_spec(
                            "src.mcp_server.tools._healthcare.clinical_timeline"
                        )
                        demographics_spec = importlib.util.find_spec(
                            "src.mcp_server.tools._healthcare.demographics"
                        )
                        if timeline_spec is None or demographics_spec is None:
                            missing_deps.append("Patient tool dependencies not available")

                    elif tool_name == "AnalyticsTools":
                        # Test if dependencies can be imported
                        condition_spec = importlib.util.find_spec(
                            "src.mcp_server.tools._healthcare.clinical_data.condition_analytics"
                        )
                        financial_spec = importlib.util.find_spec(
                            "src.mcp_server.tools._healthcare.financial"
                        )
                        if condition_spec is None or financial_spec is None:
                            missing_deps.append("Analytics tool dependencies not available")
